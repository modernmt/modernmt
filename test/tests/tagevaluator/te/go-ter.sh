...
if [[ $option = "none" ]]; then
    cat $ref | perl $bindir/changeShiftCharacters.pl |\
               perl $bindir/compactTags.pl |\
               awk '{print $0,"(0.0-"c++")"}' > __ref__$$
    cat $hyp | perl $bindir/changeShiftCharacters.pl |\
               perl $bindir/compactTags.pl |\
               awk '{print $0,"(0.0-"c++")"}' > __hyp__$$

elif [[ $option = "separate" ]]; then
    cat $ref | perl $bindir/changeShiftCharacters.pl |\
               perl $bindir/addSpacesAroundTags.pl |\
               perl $bindir/compactTags.pl |\
               awk '{print $0,"(0.0-"c++")"}' > __ref__$$
    cat $hyp | perl $bindir/changeShiftCharacters.pl |\
               perl $bindir/addSpacesAroundTags.pl |\
...
## ORIGINAL CODE
#
# java -Dfile.encoding=UTF8 -jar $javadir/tercom.7.25.jar -s -r __ref__$$ -h __hyp__$$ -n $of
# rm __ref__$$ __hyp__$$
# 
# perl $bindir/evalTags.pl ${of}.pra_more $verbose
## -------------
