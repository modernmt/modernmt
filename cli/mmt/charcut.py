...
def read_gz8(filename):
    ...
    with opener(filename, 'r', encoding='utf-8') as f:
...
def load_input_files(args):
    ...
    #     print ("load_input_files before tokenization_international type(cand):{}".format(type(cand)))
    #     c = tokenization_international(cand)
    #     print("load_input_files after  tokenization_international type(cand):{}".format(type(c)))
    #     # exit(1)
    #
    #
    # for ref in ref_segs:
    #     print ("load_input_files before tokenization_international type(ref):{}".format(type(ref)))
    #     r = tokenization_international(ref)
    #     print("load_input_files after  tokenization_international type(ref):{}".format(type(r)))
...
def skipped2html(skipped):
    return '''
<tr>
  <td class="detail" title="Mismatch - {skipped} seg. skipped">[...]</td>
</tr>'''.format(**locals())

def total2html(doc_cost,doc_div):
    score = (1.*doc_cost/doc_div) if doc_div else 0
    return '''
<tr>
  <th>Total</th>
  <th class="score"><span class="detail">{doc_cost:.0f}/{doc_div:.0f}=</span><br/>{score:.0%}</th>
  <th></th>
</tr>'''.format(**locals())
...
def html_dump(out_file, aligned_segs, styled_ops, seg_scores, doc_cost, doc_div):
    ...
    print(header2html(), file=out_file)
    ...
    for segs, ops, score_pair in zip(aligned_segs, styled_ops, seg_scores):
        if prev_id:
            ...
            try:
                ...
                skipped = None
            if skipped:
                print (skipped2html(skipped), file=out_file)

        prev_id = segs[0]

        print(segs2html(segs, ops, score_pair), file=out_file)

    print(total2html(doc_cost, doc_div), file=out_file)

    print(footer2html(), file=out_file)
    ...
    if getattr(args, 'plain_output_file', None):
        with open(args.plain_output_file, 'w') as plain_file:
            for pair in seg_scores:
                print(format_score(*pair,args.verbose), file=plain_file)
...
def corpus_charcut(reference, hypothesis, tokenize=True, alt_norm=False, match_size=3):

    if tokenize:
        cand_segs = [tokenization_international(x) for x in hypothesis]
        ref_segs = [tokenization_international(x) for x in reference]

    aligned_segs = [(i, None, None, cand.strip(), ref.strip())
            for i, (cand, ref) in enumerate(zip(cand_segs, ref_segs), 0)]

    styled_ops = [compare_segments(cand, ref, match_size)
                  for seg_id, _, _, cand, ref in aligned_segs]
...
def tokenization_international(line):
    ...
    if type(norm_text) is bytes:
        norm_text = str(norm_text, 'utf-8')
    ...
    for i in range(0, len(tagList)):
        norm_text = re.sub('MTEVALXMLTAG%d' % i, tagList[i], norm_text, 1, flags=re.U)
