...
bool ParseArgs(int argc, const char *argv[], args_t *args) {
    ...
    options.add_options()
            ...
            ("model", po::value<string>()->required(), "RocksLM model path")
    ...
    try {
        ...
        if (vm.count("help")) {
            cerr << "RocksLM is an incremental, adaptive language model built with RocksDB. Use this binary "
...
int main(int argc, const char *argv[]) {
    ...
    RocksLM lm(args.model_path, options);
