...
#ifndef ROCKSLM_ADAPTIVELM_H
#define ROCKSLM_ADAPTIVELM_H
...
namespace rockslm {
    ...
    public:
        ...
        virtual void Add(const updateid_t &id, domain_t domain, const vector<wid_t> &source, const vector<wid_t> &target,
    ...
    private:
        ...
        cachevalue_t ComputeUnigramProbability(const context_t *context, db::dbkey_t wordKey) const;

        inline db::count_t OOVClassFrequency(const db::count_t dictionarySize) const;

        inline db::count_t OOVClassSize(const db::count_t dictionarySize) const;

    };

}

#endif //ROCKSLM_ADAPTIVELM_H
