...
namespace rockslm {
    ...
    // The state of RocksLM is always the combination of the static lm state and
    ...
    struct RocksLMHistoryKey : public HistoryKey {

        const HistoryKey *alm_key;
        const HistoryKey *slm_key;

        RocksLMHistoryKey(HistoryKey *alm_key, HistoryKey *slm_key) : alm_key(alm_key), slm_key(slm_key) {}
        ...
        bool operator==(const HistoryKey &other) const override {
            RocksLMHistoryKey &o = (RocksLMHistoryKey &) other;
            if (alm_key == NULL)
...
struct RocksLM::rockslm_private {
...
RocksLM::RocksLM(const string &modelPath, const Options &options) {
...
HistoryKey *RocksLM::MakeHistoryKey(const vector<wid_t> &phrase) const {
    return new RocksLMHistoryKey(self->alm ? self->alm->MakeHistoryKey(phrase) : NULL,
                                 self->slm ? self->slm->MakeHistoryKey(phrase) : NULL);
}

HistoryKey *RocksLM::MakeEmptyHistoryKey() const {
    return new RocksLMHistoryKey(self->alm ? self->alm->MakeEmptyHistoryKey() : NULL,
                                 self->slm ? self->slm->MakeEmptyHistoryKey() : NULL);
}

bool RocksLM::IsOOV(const context_t *context, const wid_t word) const {
...
float RocksLM::ComputeProbability(const wid_t word, const HistoryKey *historyKey, const context_t *context,
                                  HistoryKey **outHistoryKey, void *cache) const {
    const RocksLMHistoryKey *inKey = static_cast<const RocksLMHistoryKey *> (historyKey);
    assert(inKey != NULL);
    ...
    if (outHistoryKey)
        *outHistoryKey = new RocksLMHistoryKey(alm_key, slm_key);

    return (float) result;
}

void RocksLM::Add(const updateid_t &id, const domain_t domain, const vector<wid_t> &source, const vector<wid_t> &target,
...
void RocksLM::NormalizeContext(context_t *context) {
    if (self->is_alm_active)
