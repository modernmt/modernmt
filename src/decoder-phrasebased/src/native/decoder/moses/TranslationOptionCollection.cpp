 ...
 bool adhereTableLimit, size_t gidx, InputPath &inputPath)
{
  ...
  if ((xml_policy != XmlExclusive)
    ...
    PartialTranslOptColl* oldPtoc = new PartialTranslOptColl(m_maxNoTransOptPerCoverage);
    ...
    for (++d ; d != dgraph.end() ; ++d) {
      const DecodeStep *dstep = *d;
      PartialTranslOptColl* newPtoc = new PartialTranslOptColl(m_maxNoTransOptPerCoverage);
