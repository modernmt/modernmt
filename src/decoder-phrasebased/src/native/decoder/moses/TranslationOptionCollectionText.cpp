...
//, size_t maxNoTransOptPerCoverage, float translationOptionThreshold)
    ...
    // , maxNoTransOptPerCoverage, translationOptionThreshold)
    {
      ...
      for (size_t startPos = 0; startPos < size; ++startPos) {
        for (size_t phraseSize = 1; phraseSize <= size - startPos; ++phraseSize) {
 ...
 */
    bool TranslationOptionCollectionText::HasXmlOptionsOverlappingRange(size_t startPosition,
 ...
 */
    bool TranslationOptionCollectionText::ViolatesXmlOptionsConstraint(size_t startPosition, size_t endPosition,
                                                                       TranslationOption *transOpt) const {
      ...
      for (size_t i = 0; i < xmlOptions.size(); i++) {
        ...
        if ((range.GetStartPos() < startPosition && range.GetEndPos() >= startPosition &&
             range.GetEndPos() < endPosition) ||
            (range.GetEndPos() > endPosition && range.GetStartPos() <= endPosition &&
             range.GetStartPos() > startPosition)) {
          return true;
        }
