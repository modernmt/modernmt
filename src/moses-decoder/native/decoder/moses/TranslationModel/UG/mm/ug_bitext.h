...
namespace sapt
{
    ...
    bool const flip) const     // flip source and target (reverse lookup)
  {
    ...
    for (Token const* T1_k = T1->sntStart(sid); T1_k < T1->sntEnd(sid); ++T1_k){ VERBOSE(2, T1_k->id() << " "); }
    VERBOSE(2, "||| ");
    for (Token const* T2_k = T2->sntStart(sid) ; T2_k < T2->sntEnd(sid); ++T2_k){ VERBOSE(2, T2_k->id() << " "); }
    VERBOSE(2, endl);
    ...
    while (p < x)
      {
        ...
        //VERBOSE(2, "bool  Bitext<Token>::::find_trg_phr_bounds(size_t ...) src=" << src << " start=" << start << " stop=" << stop << " lft=" << lft << " rgt=" << rgt << endl);
        if (src < start || src >= stop) {
          //VERBOSE(2, "bool  Bitext<Token>::::find_trg_phr_bounds(size_t ...) IF " << endl);
          forbidden.set(trg);
        }
        else
          {
            //VERBOSE(2, "bool  Bitext<Token>::::find_trg_phr_bounds(size_t ...) ELSE " << endl);
            lft = std::min(lft,trg);
            rgt = std::max(rgt,trg);
          }
        if (core_alignment)
